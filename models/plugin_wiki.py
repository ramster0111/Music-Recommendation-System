...
DEFAULT = {
    ...
    'theme'  : 'ui-darkness' # the jquery-ui theme, mapped into plugin_wiki/ui/%(theme)s/jquery-ui-1.8.1.custom.css
    }
...
###################################################
for _f in ['plugin_wiki/ui/%s/jquery-ui-1.8.1.custom.css' % plugin_wiki_theme,
...
""" % dict(callback=callback,colnames=json(colnames),
    ...
    def slideshow(table,field='image',transition='fade',width=200,height=200):
        ...
        if db[table][field].type=='upload':
            images = [IMG(_src=URL(r=request,c='default',d='download',args=row[field])) for row in rows]
        else:
